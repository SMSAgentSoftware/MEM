############################################
## SOFTWARE UPDATES DATA GATHERING SCRIPT ##
############################################

###### VARIABLES - SET THESE
$ProgressPreference = 'SilentlyContinue'
$FullInventorySchedule = 1 # DAYS - MINIMUM number of days between sending a full inventory
$DeltaInventorySchedule = 1 # HOURS - MINIMUM number of hours between sending a delta inventory
$WorkspaceID = "<YOUR WORKSPACE GUID>" # WorkspaceID of the Log Analytics workspace
$PrimaryKey = "<YOUR WORKSPACE PRIMARY KEY>" # Primary Key of the Log Analytics workspace
$ParentDirectoryName = "<YOUR COMPANY NAME>" # eg Company or domain name
######
...
# try some other ways to get the AaddeviceId in case the cert is missing somehow
else 
{
    ...
    If ($null -eq $AadDeviceId)
    {
        try 
        {
            ...
            {
                $AADDeviceID = $DeviceIdMatch.Line.Split()[-1]
            } 
        }
        catch {}
    }
...
{
    # for legacy W10 where DisplayVersion wasn't a thing yet
    switch ($CurrentBuild)
    {
...
If ($InventoryType -eq "Delta")
{
    ...
    If ($LatestFullInventoryFile)
    {
        ...
        else 
        {
            ...
            {       
                # Identify which sections are different and create a delta file containing just the changes
